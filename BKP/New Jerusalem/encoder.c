...
void write_to_output(FILE* outFile, struct encoder* enc, short int dynamic){
    long int i;
...
//Codifica o arquivo
void encode(FILE* inFile, FILE* outFile, struct encoder* enc, short int dynamic){
    printf("\nEncoding...");
    ...
    while (!feof(inFile)){
        ...
        if (strlen(enc->strBuffer) == 0 || foundCode >=0){
			...
			if (dynamic && floor(log2(enc->nextCode)) == 8+updateIndex){ //8 bits + o número de incrementos - TRAB 2
			...
			//Print indicativo de que está codificando
			if(enc->nextCode % (unsigned int)(pow(2, 10)*10) == 0) printf(".");
        }
    }
    //Determina o tamanho das palavras no dicionário
    if(!dynamic) enc->indexBits = ceil(log2(enc->nextCode-1)); //Apenas TRAB 1
    //Escreve para o arquivo de saída
    write_to_output(outFile, enc, dynamic);
    printf("\nEncoded!\n");
}
